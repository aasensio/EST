...
class trainCNN(object):
    ...
    def extractTrainingData2(self):
        ...
        self.XTestSet = np.zeros((300,9*nRows,self.patchSize,self.patchSize))
        self.YTestSet = np.zeros((300,9*nRows))
        ...
        self.YTrainSet = self.YTrainSet.reshape((300*9*nRows,1))

        self.XTestSet = self.XTestSet.reshape((300*9*nRows,1,self.patchSize,self.patchSize))
        self.YTestSet = self.YTestSet.reshape((300*9*nRows,1))
...
# Normalization 2
    ...
    def defineFully(self):
        self.model = Sequential()
        self.model.add(Flatten(input_shape=(1, self.patchSize, self.patchSize)))
        ...
        self.model.compile(loss='mae', optimizer='RMSprop')

    def trainCNN(self, nIterations):
        print("Training network...")
        self.metrics = self.model.fit(self.XTrainSet, self.YTrainSet, batch_size=self.batchSize, nb_epoch=nIterations, validation_data=(self.XTestSet, self.YTestSet), shuffle=True)
        # self.model.fit(self.XTrainSet, self.YTrainSet, batch_size=self.batchSize, nb_epoch=self.nbEpoch, validation_split=0.2)

    def testCNN(self):
        train = self.model.predict(self.XTrainSet)
