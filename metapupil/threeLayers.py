...
for k in range(nHeights):
    for j in range(nHeights):
        for i in range(nHeights):
            if ((testHeights[i] > testHeights[j]) and (testHeights[j] > testHeights[k])):
                forward.generateTurbulentZernikesKolmogorov(r0, layers=[testHeights[k],testHeights[j],testHeights[i]], percentages=[65.0, 25.0, 10.0])
                bMeasured = forward.generateWFS()
                bMeasured += np.random.normal(scale=0.01, size=bMeasured.shape)
                
                inversion = tomography.tomography(nStars, nZernike, fov, heights, DTel)
                inversion.generateTurbulentZernikesKolmogorov(r0, layers=[testHeights[k],testHeights[j],testHeights[i]], percentages=[65.0, 25.0, 10.0])
                inversion.solveSVD(bMeasured, regularize=True)

                # inversion.solveFASTA(bMeasured, mu=0.001)

                inversion.solveBSBL(bMeasured, lambdaPar=0.00001)
...
np.savez('results/threeLayer.npz', original, reconstructed)