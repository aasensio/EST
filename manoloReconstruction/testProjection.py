...
metapupil *= 0.0
for i in range(nMax):
    z, n, m = wf.zernike(i+1, npix=int(2*radius))
    metapupil += a[i] * z
...
# M = projection.zernikeProjectionMatrix(nMax, beta, t, alphat, includePiston=True)
M2 = projection.zernikeProjectionMatrixNumerical(nMax, beta, t, alphat, includePiston=True, radius=512)
...
for i in range(nMax):
    z, n, m = wf.zernike(i+1, npix=int(2*radiusCircle))
    footprint += b[i] * z
...
ps.plot.tvframe(metapupil * mask, ax=ax[0])
ax[0].add_artist(circle)
ps.plot.tvframe(maskedMetaPupil, ax=ax[1])
ps.plot.tvframe(footprint, ax=ax[2])
