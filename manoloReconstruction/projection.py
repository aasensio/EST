...
def zernikeProjectionMatrix(nMax, beta, t, alphat, verbose=True, includePiston=False):
    ...
    for i in range(nMax):    
        for j in range(nMax):       
            if (verbose):
                ps.util.progressbar(loop, nMax*nMax)
...
def zernikeProjectionMatrixNumerical(nMax, beta, t, alphat, radius=128, verbose=True, includePiston=False):
    ...
    xCircleMask = round(radius + radius * t * np.sin(alphat))
    yCircleMask = round(radius + radius * t * np.cos(alphat))
    mask = cmask(xCircleMask, yCircleMask, radiusCircle, zero)
    
    zi = []
    zj = []
    for j in range(nMax):
        ziTemp, n, m = wf.zernike(j+delta, npix=int(2*radius))
        zjTemp, n, m = wf.zernike(j+delta, npix=int(2*radiusCircle))
        zi.append(ziTemp)
        zj.append(zjTemp)    
    ...
    for i in range(nMax):
        ...
        for j in range(nMax):       
            ...
            M[j,i] = np.sum(zj[j] * maskedMetaPupil) / np.sum(zj[j]**2)
            loop += 1
